...
gcam_output_dir <- "~/Documents/GCAM-WD/CMPs/CMP406/Gv8-Hv3.2"
...
if(TRUE){
    ...
    inputs <- get_hector_inputs(db_dir = gcam_output_dir,
                                db_name = db_name,
                                prj_file = prj_file)
    write.csv(inputs, "scripts/example_inputs-v82.csv", row.names = TRUE)
    ...
    out <- run_GCAM2hector(db_dir = gcam_output_dir,
                           db_name = db_name,
                           prj_file = prj_file)
    write.csv(out, "scripts/example_hector-v82.csv", row.names = TRUE)


    gcam_hector_rslts <- fetch_GCAM_vs_hector(prj_file)
    write.csv(out, "scripts/gcam_hector_rslts-v82.csv", row.names = TRUE)
...
out %>%
    filter(year %in% 1975:2002) %>%
    summarise(hist_mean = mean(value), .by = c("scenario", "variable")) ->
    hist
...
rbind(gcam_hector_rslts, out) %>%
    ...
    #filter(year <= 2005) %>%
    summarise(MAE = mean(abs(AE)), .by = c("variable", "scenario")) %>%
    left_join(hist) %>%
    mutate(ratio = MAE/hist_mean) %>%
